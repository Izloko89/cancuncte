...
function guardar_art(elemento){
	...
	if(id_cotizacion!=""){
		id_cotizacion = document.getElementById("clave").value;
...
function saveImage()
{
	...
	$.each($('#file')[0].files, function(i, file) {
		...
		data.append('cotizacion', id_cotizacion);
	});
	$.ajax({
		url:'scripts/saveImageCot.php',
		data: data,	
		cache: false,
		contentType: false,
		processData: false,
	...
	$.ajax({
	  ...
	  success: function(r){
		...
		if(r.bool){
			...
			document.getElementById("imp_cot1").setAttribute("href", "scripts/pdf_cotizacion2.php?id=" + r.id_cotizacion);
	...
	if(continuar){
		...
		$.ajax({
			...
			success: function(r){
				...
				//	 window.location="eventos.php?cve="+r.id;
				}else{
					alerta("error",r.info);
				}
			}
