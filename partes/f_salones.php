...
<?php
	try{
		...
		foreach($res->fetchAll(PDO::FETCH_ASSOC) as $v){
			echo '<tr class="salon fondo_azul" ><td ><div align="left" >'.$v["nombre"].'</div></td><td colspan="2" align="right"><span class="eliminar_salon" onclick="eliminar_art(' . $cont . ', ' . $v["id_salon"] . ');"> <input id="idTipo" type="hidden" value="' . $v["id_salon"] . '"/></td></tr>';
			$cont++;
		}
