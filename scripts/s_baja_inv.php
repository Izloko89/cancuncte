...
try{
	...
	//1.- checar si ya existe en el almacén
		...
		$res1=$bd->query("SELECT id_item, cantidad FROM almacen_inventario WHERE id_empresa=$id_emp AND id_articulo=$id_art;");
		if($res->rowCount()>0){
			...
			$cantidadPrevia1=$res1[0]["cantidad"];
			$nuevaCant=$cantidadPrevia-$cant;
			$nuevaCant1=$cantidadPrevia1-$cant;
			$bd->query("UPDATE almacen SET cantidad='$nuevaCant' WHERE id_item=$id_item;");
			$bd->query("UPDATE almacen_inventario SET cantidad='$nuevaCant1' WHERE id_item=$id_item;");
	}else{
	...
	VALUES ($id_item,$id_emp,$id_art,'$paraInv') 
	ON DUPLICATE KEY UPDATE cantidad='$paraInv';");
	$r["continuar"]=true;
