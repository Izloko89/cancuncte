...
if($id_cot!=""){
	try{
		...
		///////////////////////7
		                                    // selecciono el path de la imagen segun el id de la cotizacion
		$sql="SELECT 
			path as path
		...
		$res=$res->fetchAll(PDO::FETCH_ASSOC);	
		$path=$res[0]["path"];		
		// como ya tengo el id del evento solo le paso el path y el id del evento a la tabla evento_imagen
		
		
		///////////////////////////////////
		
		
		$sql="INSERT INTO evento_imagen (path,id_evento) values ('$path',$id_eve)  ;";
		$bd->query($sql);
		...
		*/
		...
		foreach($res->fetchAll(PDO::FETCH_ASSOC) as $dato)
		{
			...
			if($dato["cantidad"] > $cantidadexistencia)
			{
				...
				// $bd->query($sql);
				// $r["info"]="<strong>guardado exitosamente</strong>";
			}
		}
