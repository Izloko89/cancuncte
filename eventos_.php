  ...
  <div id="hacer">
    ...
    <h3 class='titulo_form'>Cuenta</h3>
        ...
        <div class="campo_form">
            ...
            <input type="text" class="restante numerico input_sin_color" readonly="readonly" />
        </div>
        <div align="right">
...
<input type="submit" onclick = "this.form.action = 'scripts/nota_venta_pdf.php'" value="Imprimir" class="flota_der" />
        <!--</form>
        <form action="scripts/pdf_contrato.php" style="display:inline" target="_blank">
        <input type="hidden" name="id_evento" class="id_evento" value="" />
        <input type="hidden" name="obs" id="observaciones"/></br />
        <input type="submit" value="Contrato" class="flota_der"/>
        </form>-->
        </form>
    </div>
  </div>
<!-- //sección de las eventos por empresa y or usuario -->
  ...
  <tr class="barra_accion">
	...
	try{
		...
		WHERE eventos.id_empresa=$empresaid ORDER BY clave ASC;";
		$sqlClie="SELECT
			id_evento,
			clientes.id_cliente,
			clientes.nombre,
